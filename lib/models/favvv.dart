...
Product(imageURL: 'imgf/51Ykk5s9wJL._AC_UY1000_-removebg-preview.png', name: 'Fancy Slippers', price: 299, description: 'White border slippers'),
  ...
  List<Product> userFav = [];
  //get list of shoes for sale
  List<Product> getshoelist(){
    return FavShop;
  }
  //get fav
  List<Product> getuserFav(){
    return userFav;
  }
  // add items in the fav
  void additemstoFav(Product product){
    userFav.add(product);
    notifyListeners();
  }
  // remove items in the cart
  void removeitemfromFav(Product product){
    userFav.remove(product);
    notifyListeners();
  }
}